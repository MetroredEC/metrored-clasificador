...
async function extractCasesWithClaude(pdfBase64, apiKey) {
  const prompt = `Analiza este PDF de documentos medicos de Metrored / BUPA Ecuador.

El PDF contiene multiples CASOS. Cada caso normalmente tiene 2 paginas:
  1. Estado de Cuenta de METRORED (logo Metrored arriba, datos del paciente)
  2. Autorizacion de Cobertura de BUPA (logo BUPA arriba, numero de poliza entre parentesis)

Para cada caso extrae:
- policy_number : El numero de 6 digitos entre parentesis en la pagina BUPA.
                  Ejemplo: de "(700220)" extrae "700220". Si no hay BUPA, usa "SIN_POLIZA".
- patient_name  : Nombre completo en MAYUSCULAS. Campo "Asegurado:" en BUPA o "Paciente:" en Metrored.
- pages         : Array de numeros de pagina (iniciando en 1) que pertenecen a este caso.
- document_types: Array con tipo de cada pagina:
                  "estado_cuenta_metrored" o "autorizacion_cobertura_bupa" o "otro_documento"

REGLAS:
- Si un par de paginas pertenecen al mismo paciente, agrupalas en un solo caso.
- Ignora paginas en blanco o ilegibles.
- Devuelve UNICAMENTE el array JSON valido, sin texto extra, sin markdown, sin comillas de codigo.
...
[{"policy_number":"700220","patient_name":"RAMIREZ ANCHUNDIA EDWARD FABRICIO","pages":[1,2],"document_types":["estado_cuenta_metrored","autorizacion_cobertura_bupa"]}]`;
  ...
  // GPT-4o: send PDF pages as images via base64
  ...
  try {
    ...
    throw new Error(`Respuesta de IA no es JSON valido: ${cleaned.substring(0, 200)}`);
  }
}
